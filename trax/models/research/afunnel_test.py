...
from trax.models.research.funnel_transformer import AFunnel
...
class FunnelTransformerTest(parameterized.TestCase):
    ...
    def test_ufunnel_forward_shape_flat(self):
        vocab_size = 16
        tokens = 3*2*2*2*2*2
        model = AFunnel(vocab_size, narrow_segment_lengths=(2,), wide_segment_lengths=(2,2))
    ...
    def test_ufunnel_forward_shape_deep(self):
        ...
        for segments in [
            ...
            model = AFunnel(vocab_size, narrow_segment_lengths=[*segments[1:]], wide_segment_lengths=segments)
            x = np.ones((3, tokens)).astype(np.int32)
            _, _ = model.init(shapes.signature(x))
            y = model(x)
            self.assertEqual(y.shape, (3, tokens, vocab_size))
