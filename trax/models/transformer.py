...
def FunnelTransformer(input_vocab_size,
  ...
  assert(d_model % 2**n_encoder_layers == 0) # TODO: raise
  assert(d_model % 2**n_encoder_layers == 0) # TODO: raise

  encoder_blocks = [
      _EncoderBlock(d_model/2**i, d_ff/2**i, n_heads, dropout, dropout_shared_axes,
                    mode, ff_activation)
